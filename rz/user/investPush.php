...
//上传文件的路径
$dir = substr(dirname(__FILE__), 0, -4)."img\invest";
/*
...
$query = "insert into invest(title,content,mobile,time,name,contentAdv,remarks,money) values ('".$title."','".$content."','".$mobile."','".$time."','".$_SESSION['name']."','".$content1."','".$content2."','".$money."')";
mysql_query($query , $db) or die(mysql_error($db));
$last_id = mysql_insert_id();
//用写入的id作为图片的名字，避免同名的文件存放在同一目录中
$imagename = $last_id.$ext;
$query = 'update invest set imgName="'.$imagename.'" where ID='.$last_id;
mysql_query($query , $db) or die(mysql_error($db));
