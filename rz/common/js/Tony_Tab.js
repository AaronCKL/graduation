...
jQuery.fn.extend({
	...
	slideFocus: function(){
		...
		// btnn event
		var btnn = "<div class='btnn'>";
		for(var i=0; i < len; i++) {
			btnn += "<span></span>";
		};
		btnn += "</div><div class='preNext pre'></div><div class='preNext next'></div>";
		$(This).append(btnn);
		$(This).find('.btnn span').eq(0).addClass('on');
		$(This).find('.btnn span').mouseover(function(){
			index = $(This).find('.btnn span').index(this);
			Tony(index);
		});
      var touch = {};
	  var min=30;
      $(This).on('touchstart', function(e) {
       var touches = e.originalEvent.touches[0];//touches[0]改为originalEvent.touches[0]，touches[0]有bug
		...
		function Tony(index){
			...
			$(This).find('.btnn span').stop(true,false).eq(index).addClass('on').siblings().removeClass('on');
		};
		//siblings()返回所有同胞元素
